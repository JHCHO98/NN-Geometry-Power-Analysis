...
<main>
  ...
  <section id="pareto" class="panel active">
    ...
    <div class="chart-actions"><input id="pareto-export-name" aria-label="Pareto PNG filename" value="accuracy-energy-pareto" spellcheck="false"><button type="button" data-export="pareto-canvas" data-name-input="pareto-export-name" data-filename="accuracy-energy-pareto">Export PNG</button><button type="button" data-reset="pareto-canvas">Reset view</button></div><p class="note">Wheel or keyboard = / −: both axes · Shift: X axis only · Alt: Y axis only</p>
  ...
  <section id="scatter" class="panel">
    ...
    <div class="chart-actions"><input id="custom-export-name" aria-label="Custom scatter PNG filename" value="custom-scatter" spellcheck="false"><button type="button" data-export="custom-canvas" data-name-input="custom-export-name" data-filename="custom-scatter">Export PNG</button><button type="button" data-reset="custom-canvas">Reset view</button></div><p class="note">Wheel or keyboard = / −: both axes · Shift: X axis only · Alt: Y axis only</p><p class="status" id="custom-status" aria-live="polite"></p>
  </section>
</main>
<script>
let DATA = [];
...
function zoomChart(canvas,zoomIn,shiftKey=false,altKey=false){const state=CHART_VIEWS.get(canvas);if(!state)return;const scale=zoomIn?.82:1.22,xd=state.xDomain,yd=state.yDomain,xCenter=(xd[0]+xd[1])/2,yCenter=(yd[0]+yd[1])/2;if(!altKey){const span=(xd[1]-xd[0])*scale;state.xDomain=[xCenter-span/2,xCenter+span/2];}if(!shiftKey){const span=(yd[1]-yd[0])*scale;state.yDomain=[yCenter-span/2,yCenter+span/2];}redrawSoon(canvas);}
...
document.addEventListener('keydown',event=>{const tag=document.activeElement?.tagName;if(['INPUT','SELECT','TEXTAREA'].includes(tag)||event.ctrlKey||event.metaKey)return;const zoomIn=event.code==='Equal'||event.code==='NumpadAdd',zoomOut=event.code==='Minus'||event.code==='NumpadSubtract';if(!zoomIn&&!zoomOut)return;const canvas=document.getElementById(document.getElementById('pareto').classList.contains('active')?'pareto-canvas':'custom-canvas');zoomChart(canvas,zoomIn,event.shiftKey,event.altKey);event.preventDefault();});
